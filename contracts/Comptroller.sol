...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when cap guardian is changed
    ...
    function mintAllowed(address cToken, address minter, uint mintAmount) external override returns (uint) {
        ...
        if (supplyCap != 0) {
            Exp memory exchangeRate = Exp({mantissa: CToken(cToken).exchangeRateStored()});
            uint supplyTokenInUnderlying = mul_ScalarTruncate(exchangeRate, supplyTokens);
            uint nextTotalSupply = add_(supplyTokenInUnderlying, mintAmount);
            require(nextTotalSupply < supplyCap, "market supply cap reached");
        }

        uint supplierTokens = CToken(cToken).balanceOf(minter);

        // Keep the flywheel moving
        updateCompSupplyIndex(cToken, supplyTokens);
        distributeSupplierComp(cToken, minter, supplierTokens);
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) external override returns (uint) {
        ...
        (Error err1, , uint shortfall) = getHypotheticalAccountLiquidityInternal(borrower, CToken(cToken), 0, borrowAmount);
     ...
     */
    function liquidateBorrowAllowed(address cTokenBorrowed, address cTokenCollateral, address liquidator, address borrower, uint repayAmount) external override returns (uint) {
        // Shh - currently unused
        // liquidator;
     ...
     */
    function seizeAllowed(address cTokenCollateral, address cTokenBorrowed, address liquidator, address borrower, uint seizeTokens) external override returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        ...
        uint supplyTokens = CToken(cTokenCollateral).totalSupply();
        uint borrowerTokens = CToken(cTokenCollateral).balanceOf(borrower);
        uint liquidatorTokens = CToken(cTokenCollateral).balanceOf(liquidator);

        // Keep the flywheel moving
        updateCompSupplyIndex(cTokenCollateral, supplyTokens);
        distributeSupplierComp(cTokenCollateral, borrower, borrowerTokens);
        distributeSupplierComp(cTokenCollateral, liquidator, liquidatorTokens);
     ...
     * @dev Admin or capGuardian function to set the supply caps. A suppply cap of 0 corresponds to unlimited supplying.
     ...
     */
    function _setCapGuardian(address newCapGuardian) external {
        require(msg.sender == admin, "only admin can set cap guardian");

        // Save current value for inclusion in log
        address oldCapGuardian = capGuardian;

        // Store capGuardian with value newCapGuardian
    ...
    function setCompSpeedInternal(CToken cToken, uint supplySpeed, uint borrowSpeed) internal {
        ...
        if (compSupplySpeeds[address(cToken)] != supplySpeed) {
            ...
            updateCompSupplyIndex(address(cToken), supplyTokens);
     ...
     */
    function updateCompSupplyIndex(address cToken, uint supplyTokens) internal {
        CompMarketState storage supplyState = compSupplyState[cToken];
     ...
     */
    function distributeSupplierComp(address cToken, address supplier, uint supplierTokens) internal {
    ...
    function claimComp(address[] memory holders, CToken[] memory cTokens, bool borrowers, bool suppliers) public {
        for (uint i = 0; i < cTokens.length; i++) {
            ...
            if (suppliers == true) {
                ...
                updateCompSupplyIndex(address(cToken), supplyTokens);
                for (uint j = 0; j < holders.length; j++) {
                    uint supplierTokens = cToken.balanceOf(holders[j]);
                    distributeSupplierComp(address(cToken), holders[j], supplierTokens);
                }
            }
        }
        for (uint j = 0; j < holders.length; j++) {
    ...
    function isDeprecated(CToken cToken) public view returns (bool) {
        return markets[address(cToken)].collateralFactorMantissa == 0 && borrowGuardianPaused[address(cToken)] == true && cToken.reserveFactorMantissa() == 1e18;
    }
     ...
     */
    function setCompAddress(address newAddress) public {
        require(msg.sender == admin, "Only admin can call");
        compAddress = newAddress;
    }
    ...
    function getBorrowersCount() public view returns (uint) {
        return allBorrowers.length;
    }

    function getBorrowers(uint startIndex, uint endIndex) public view returns (address[] memory) {
        require(startIndex < endIndex && endIndex <= allBorrowers.length, "Invalid slice range");

        address[] memory slice = new address[](endIndex - startIndex);
        for (uint i = startIndex; i < endIndex; i++) {
            slice[i - startIndex] = allBorrowers[i];
        }
        return slice;
    }
}
