...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
    ...
    /**
     * @notice Sets a new protocol seize share (taken from liquidation) for the protocol.
     * @dev Admin function to set a new protocol seize share
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setProtocolSeizeShare(uint newProtocolSeizeShareMantissa) external override nonReentrant returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
